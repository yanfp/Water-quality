...
{
    public partial class MahalanobisDistance : DevExpress.XtraEditors.XtraForm
    {
        List<string> MahalanobisDistName = new List<string>();
        ...
        private void MahalanobisDistance_Load(object sender, EventArgs e)
        {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string str = ".compile '" + Application.StartupPath + "\\RSImage_MahalanobisDistance.pro" + "'";
                oComIDL.ExecuteString(str);

                string inputfile = string.Empty;
                for (int i = 0; i < MahalanobisDistName.Count(); i++)
                {
                    string selectedFileName = cmbInputFile.SelectedItem.ToString().Trim();
                    if (selectedFileName == System.IO.Path.GetFileName(MahalanobisDistName[i]))
                    {
                        inputfile = MahalanobisDistName[i];
                    }
                    else
                    {
                ...
                string str1 = "RSImage_MahalanobisDistance" + ",'" + inputfile + "'" + "," + pos + ",'" + txtInputROI.Text + "'" + ",'" + txtOutputRuleFile.Text + "'" + ",'" + txtOutputImage.Text + "'";
